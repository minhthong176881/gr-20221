...
class Variant extends React.Component {
  ...
  componentDidMount() {
    axios
      .get(`${config.host}/variant?limit=${config.pageSize}&from=0`)
      .then((res) => {
        this.setState({
          total: res.data.total_hits,
          items: formatResponse(res.data.variants),
        });
      });
  }
          ...
          <Table
            ...
            pagination={{
              ...
              onChange: (page, pageSize) => {
                axios
                  .get(
                    `${config.host}/variant?limit=${pageSize}&from=${
                      (page - 1) * pageSize
                    }`
                  )
                  .then((res) => {
              ...
              onShowSizeChange: (current, size) => {
                axios
                  .get(
                    `${config.host}/variant?limit=${size}&from=${
                      (current - 1) * size
                    }`
                  )
                  .then((res) => {
